...
function parseBikesFromHtml(html, state, baseUrl) {
  ...
  $('a[href]').each((_, el) => {
    ...
    if (!fullHref || (!/\/fietsen\//i.test(fullHref) && !/detail/i.test(fullHref))) {
      return;
    }

    const scope = a.closest('article, li, .item, .product, .fiets, .bike, .card, .block, .result, .row, .col').first();
    ...
    const image = absUrl(firstNonEmpty([
    ...
    const price = normalizePrice(firstNonEmpty([
      ...
      node.find('.price,.prijs,.amount,.product-price,.sale-price,.prijsblok,.price-box').first().text(),
      a.find('.price,.prijs').first().text(),
      blockText
    ]));
...
async function enrichMissingPrices(items) {
  const missing = items.filter(item => !item.price || item.price === 'Prijs op aanvraag').slice(0, 24);
  ...
  await Promise.allSettled(missing.map(async (item) => {
    try {
      ...
      if ((!item.image || item.image.endsWith('/')) && $('img').length) {
        const image = absUrl(firstNonEmpty([
          $('.product img,.detail img,.fiets-detail img').first().attr('data-src'),
          $('.product img,.detail img,.fiets-detail img').first().attr('src'),
      ...
      if (!item.specs || !item.specs.length) {
        item.specs = extractSpecs(bodyText);
      }
    } catch (error) {
...
async function fetchCategory(url, state) {
  ...
  let items = parseBikesFromHtml(html, state, url);
  items = await enrichMissingPrices(items);
  return items;
}
...
(function() {
  ...
  if (!document.getElementById(STYLE_ID)) {
    ...
    style.id = STYLE_ID;
    style.textContent = '.bvb-embed{font-family:Inter,system-ui,sans-serif;max-width:1200px;margin:0 auto}.bvb-embed-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:20px}.bvb-embed-card{background:#fff;border:1px solid #e5e7eb;border-radius:20px;overflow:hidden;box-shadow:0 12px 30px rgba(15,23,42,.08)}.bvb-embed-media{aspect-ratio:4/3;background:#f8fafc}.bvb-embed-media img{width:100%;height:100%;object-fit:cover;display:block}.bvb-embed-body{padding:18px}.bvb-embed-pill{display:inline-block;background:#ecfdf5;color:#15803d;padding:6px 10px;border-radius:999px;font-size:12px;font-weight:700;margin-bottom:10px}.bvb-embed-pill.new{background:#eff6ff;color:#2563eb}.bvb-embed-title{font-size:18px;line-height:1.3;margin:0 0 8px;color:#0f172a}.bvb-embed-specs{font-size:14px;color:#64748b;min-height:40px;margin:0 0 14px}.bvb-embed-footer{display:flex;justify-content:space-between;align-items:center;gap:12px}.bvb-embed-price{font-size:24px;font-weight:800;color:#0f766e}.bvb-embed-btn{display:inline-block;background:#0f172a;color:#fff!important;padding:10px 14px;border-radius:12px;text-decoration:none;font-weight:700}.bvb-embed-empty{padding:28px;background:#fff;border-radius:16px;border:1px dashed #cbd5e1}';
  ...
  function render(container, fietsen){
    ...
    container.innerHTML='<div class="bvb-embed-grid">'+fietsen.map(function(f){var specs=Array.isArray(f.specs)?f.specs.join(' • '):'';var pillClass=f.state==='new'?'bvb-embed-pill new':'bvb-embed-pill';return '<article class="bvb-embed-card"><div class="bvb-embed-media">'+(f.image?'<img src="'+esc(f.image)+'" alt="'+esc(f.title)+'">':'')+'</div><div class="bvb-embed-body"><span class="'+pillClass+'">'+esc(f.stateLabel)+'</span><h3 class="bvb-embed-title">'+esc(f.title)+'</h3><p class="bvb-embed-specs">'+esc(specs)+'</p><div class="bvb-embed-footer"><div class="bvb-embed-price">'+esc(f.price)+'</div><a class="bvb-embed-btn" href="'+esc(f.url||'#')+'" target="_blank" rel="noopener noreferrer">Bekijken</a></div></div></article>';}).join('')+'</div>';
  }
